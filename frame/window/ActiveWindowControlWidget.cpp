    ...
    : QWidget(parent), mouseClicked(false)
{
    ...
    layout->addWidget(this->menuBar, 0, Qt::AlignLeft | Qt::AlignHCenter);
    // layout->addStretch();
    ...
    QTimer::singleShot(1000, this, [this]
                       { this->activeWindowInfoChanged(KWindowSystem::activeWindow()); });
...
void ActiveWindowControlWidget::initMenuBar()
{
    ...
    connect(searchEdit, &QLineEdit::textChanged, [searchTimer]
            { searchTimer->start(); });
    ...
    for (auto item : this->menuBar->actions())
    {
        if (item->menu())
        {
            connect(item->menu(), &QMenu::aboutToShow, []
                    { DockItemManager::instance()->requestWindowAutoHide(false); });
            connect(item->menu(), &QMenu::aboutToHide, []
                    { DockItemManager::instance()->requestWindowAutoHide(true); });
        }
    }
