...
const QString NetworkItem::contextMenu() const
{
    ...
    if (m_wirelessItems.size() && m_wiredItems.size()) {
        ...
        if (m_switchWiredBtnState)
            wireEnable["itemText"] = "禁用有线连接";
        else
            wireEnable["itemText"] = "开启有线连接";
        ...
        if (m_switchWirelessBtnState)
            wirelessEnable["itemText"] = "禁用无线连接";
        else
            wirelessEnable["itemText"] = "开启无线连接";
        ...
        if (m_switchWiredBtnState || m_switchWirelessBtnState)
            enable["itemText"] = "禁用网络";
        else
            enable["itemText"] = "启用网络";
...
void NetworkItem::refreshIcon()
{
    ...
    isConnection ? m_timer->start(200) : m_timer->stop();
        ...
        for (auto wirelessItem : m_connectedWirelessDevice) {
            if (wirelessItem) {
                ...
                if (m_connectedWirelessDevice.size() == 1) {
                    strTips = QString("无线连接: %1").arg(ipv4.value("Address").toString()) + '\n';
                } else {
                    strTips = QString("无线网络").append(QString("%1").arg(wirelessIndex++)).append(":"+ipv4.value("Address").toString()) + '\n';
                }
                strTips.chop(1);
                textList << strTips;
            }
        ...
        for (auto wiredItem : m_connectedWiredDevice) {
            if (wiredItem) {
                ...
                if (m_connectedWiredDevice.size() == 1) {
                    strTips = QString("有线连接: %1").arg(ipv4.value("Address").toString()) + '\n';
                } else {
                    strTips = QString("有线网络").append(QString("%1").arg(wireIndex++)).append(":"+ipv4.value("Address").toString()) + '\n';
                }
                strTips.chop(1);
                textList << strTips;
            }
