    ...
    , m_stateButton(new StateButton(this))
{
    ...
    m_securityPixmap = ImageUtil::renderSVG(pixpath, QSize(16, 16), devicePixelRatioF());
...
void AccessPointWidget::updateAP(const AccessPoint &ap)
{
    ...
    if(fontMetrics.horizontalAdvance(strSsid) > m_ssidBtn->width())
    {
    ...
    } else if(!m_securityLabel->pixmap(Qt::ReturnByValue)) {
        m_securityLabel->setPixmap(m_securityPixmap);
    }
