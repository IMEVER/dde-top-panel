...
void WiredItem::setThemeType(DGuiApplicationHelper::ColorType themeType)
{
    ...
    auto iconPix = ImageUtil::renderSVG(pixpath, QSize(PLUGIN_ICON_MAX_SIZE, PLUGIN_ICON_MAX_SIZE), devicePixelRatioF());
...
void WiredItem::changedActiveWiredConnectionInfo(const QJsonObject &connInfo)
{
    ...
    if (fontMetrics.horizontalAdvance(strTitle) > m_connectedName->width()) {
